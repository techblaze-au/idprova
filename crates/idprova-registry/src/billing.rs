...
pub async fn handle_webhook(
    ...
    // For now, parse the event body directly.
    ...
    match event_type {
        "checkout.session.completed" => {
            ...
            if let Some(session) = session {
                ...
                if !org_id.is_empty() && !customer_id.is_empty() {
                    let _ = org_store.set_stripe_customer(org_id, customer_id);
                    if !subscription_id.is_empty() {
                        let _ = org_store.set_stripe_subscription(org_id, subscription_id);
                    }
                    // Upgrade to developer tier on first checkout
                    let _ = org_store.set_tier(org_id, "developer");
