...
- OIDC ID-token verification with JWKS caching, RS256 + ES256 per RFC 0001 (Okta coexistence overlay, RFC-OVERLAY-001) §4.3.
- Deterministic OIDC-claim → IDProva trust-level + scope mapping per RFC 0001 (Okta coexistence overlay, RFC-OVERLAY-001) §6.3.
...
RFC 0001 (Okta coexistence overlay, RFC-OVERLAY-001) §6.3 defaults:
