"""IDProva AutoGen integration — scope-gated tools for ``autogen-core``.
...
The wrapped function is returned as an ``autogen_core.tools.FunctionTool``,
ready to register with an AutoGen agent.
"""
...
def guarded_function_tool(
    ...
    *,
    ...
    """Wrap ``func`` as a scope-gated ``autogen_core`` ``FunctionTool``.
    ...
    func :
        ...
        so AutoGen builds the correct tool schema.
    guard :
    ...
    autogen_core.tools.FunctionTool
        A FunctionTool wrapping the guarded function.
    """
    # Imported lazily so importing this module doesn't require autogen-core.
    ...
    def _guarded(*args: Any, **kwargs: Any) -> Any:
        # AutoGen invokes tools with keyword args; fall back to positional.
        if kwargs:
