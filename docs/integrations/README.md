...
| **LangChain (`idprova_agents.langchain_adapter`)** | **Shipped (in-repo).** `IDProvaGuardCallbackHandler` (enforce + audit-only) and `guarded_tool()` scope-gate LangChain tool calls and emit signed receipts that pass `idprova receipt verify`. Runnable example + CI-tested enforcement (incl. real CLI receipt verification). On PyPI: `pip install "idprova-agents[langchain]"`. | Package: [`sdks/python-agents/`](../../sdks/python-agents/) (`idprova-agents`). Example: [`examples/langchain/quickstart.py`](../../examples/langchain/quickstart.py). Tests: [`tests/test_langchain_quickstart.py`](../../tests/test_langchain_quickstart.py). |
...
| **CrewAI** (`crewai`) | **Shipped (in-repo).** `guarded_crewai_tool()` wraps a function as a scope-gated CrewAI `Tool`; out-of-scope calls raise and emit a signed receipt that passes `idprova receipt verify`. Runnable example + tests (verified locally; CrewAI is heavy so its tests are skipped in CI). On PyPI: `pip install "idprova-agents[crewai]"`. | Adapter: [`sdks/python-agents/idprova_agents/crewai_adapter.py`](../../sdks/python-agents/idprova_agents/crewai_adapter.py). Example: [`examples/crewai/quickstart.py`](../../examples/crewai/quickstart.py). Tests: [`tests/test_crewai_quickstart.py`](../../tests/test_crewai_quickstart.py). |
| **AutoGen** (`autogen-core`) | **Shipped (in-repo).** `guarded_function_tool()` wraps a function as a scope-gated `autogen_core` `FunctionTool`; out-of-scope calls raise and emit a signed receipt that passes `idprova receipt verify`. Runnable example + CI-tested enforcement. On PyPI: `pip install "idprova-agents[autogen]"`. | Adapter: [`sdks/python-agents/idprova_agents/autogen_adapter.py`](../../sdks/python-agents/idprova_agents/autogen_adapter.py). Example: [`examples/autogen/quickstart.py`](../../examples/autogen/quickstart.py). Tests: [`tests/test_autogen_quickstart.py`](../../tests/test_autogen_quickstart.py). |
